<template>
  <div>
    ...
    <CModal
      size="lg"
      :title="
        isUpdate ? 'Edit Batas Wilayah' : 'Tambah Batas Wilayah'
      "
      centered
    ...
    >
        <a href="batas_wilayah_import.xlsx" download class="btn btn-primary">Download format</a>
        <br /><br />
        <input
            type="file"
            class="form-control"
...
<script>
import * as data from "../../model/batas-wilayah";
...
export default {
  ...
  methods: {
    ...
    submit() {
      this.form.document_batas_kabupatens = [];
      this.imageList.forEach(element => {
        this.form.document_batas_kabupatens.push({
          image: element
        })
      });
      var loading = this.$loading.show();
      this.$store
        .dispatch("batas_wilayah/addBatasWilayah", this.form)
        .then(() => {
    ...
    edit(item) {
      ...
      this.imageList = this.extractGallery(item.document_batas_kabupatens);
    },
    cancel() {
      this.form = {
        type: 0,
    ...
    update() {
      ...
      this.$store
        .dispatch("batas_wilayah/updateBatasWilayah", {
          id: this.form.id,
          data: this.form,
        })
        .then(() => {
    ...
    hapus(item) {
      if (confirm("Data akan dihapus !!")) {
        this.$store
          .dispatch("batas_wilayah/deleteBatasWilayah", item.id)
          .then(() => {
    ...
    getData() {
      var loading = this.$loading.show();
      this.$store
        .dispatch("batas_wilayah/getBatasWilayah", this.params)
        .then((resp) => {
    ...
    selectFileImport(event) {
      ...
      var loading = this.$loading.show();
      this.$store
        .dispatch("batas_wilayah/importAlatKontruksi", this.file)
        .then((res) => {
    ...
    deleteSelected(action) {
      var loading = this.$loading.show();
      this.$store
        .dispatch("batas_wilayah/selectedAction", 
        {
          action: action,
    ...
    exportExcel(action) {
      var loading = this.$loading.show();
      this.$store
       .dispatch("batas_wilayah/exportReport", {
          data: this.selectedItems,
        })
        .then((resp) => {
          loading.hide();
    ...
    exportPDF() {
      this.$router.push({ name: "BatasWilayahExportPDF", query: { data: this.selectedItems } });
    },
    pagination(page) {
      this.page = page;
