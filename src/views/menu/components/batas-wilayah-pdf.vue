...
export default {
  ...
  methods: {
    exportPDF(id) {
      var opt = {
        margin: 2,
        filename: "Laporan_daftar_batas_wilayah.pdf",
      ...
      html2pdf()
        ...
        .from(element)
        .save()
        .then(() => this.$router.push("batas-wilayah"));
    },
    // getSigner() {
    //   var param = {
    //     sorttype: "asc",
  ...
  mounted() {
    ...
    this.$store.dispatch("batas_wilayah/ReportByDate", this.id).then((resp) => {
      loading.hide();
