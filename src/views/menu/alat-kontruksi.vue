<template>
  <div>
    ...
    <CModal
      size="lg"
      :title="
        isUpdate ? 'Edit Sewa Alat Kontruksi' : 'Tambah Sewa Alat Kontruksi'
      "
      centered
    ...
    >
        <a href="alat_kontruksi_import.xlsx" download class="btn btn-primary">Download format</a>
        <br /><br />
        <input
            type="file"
            class="form-control"
...
<script>
import * as data from "../../model/alat-kontruksi";
...
export default {
  ...
  methods: {
    ...
    submit() {
      this.form.document_alat_kontruksis = [];
      this.imageList.forEach(element => {
        this.form.document_alat_kontruksis.push({
          image: element
        })
      });
      var loading = this.$loading.show();
      this.$store
        .dispatch("alat_kontruksi/addAlatKontruksi", this.form)
        .then(() => {
    ...
    edit(item) {
      ...
      this.imageList = this.extractGallery(item.document_alat_kontruksis);
    },
    cancel() {
      this.form = {
        type: 0,
    ...
    update() {
      ...
      this.$store
        .dispatch("alat_kontruksi/updateAlatKontruksi", {
          id: this.form.id,
          data: this.form,
        })
        .then(() => {
    ...
    hapus(item) {
      if (confirm("Data akan dihapus !!")) {
        this.$store
          .dispatch("alat_kontruksi/deleteAlatKontruksi", item.id)
          .then(() => {
    ...
    getData() {
      var loading = this.$loading.show();
      this.$store
        .dispatch("alat_kontruksi/getAlatKontruksi", this.params)
        .then((resp) => {
    ...
    selectFileImport(event) {
      ...
      var loading = this.$loading.show();
      this.$store
        .dispatch("alat_kontruksi/importAlatKontruksi", this.file)
        .then((res) => {
    ...
    deleteSelected(action) {
      var loading = this.$loading.show();
      this.$store
        .dispatch("alat_kontruksi/selectedAction", 
        {
          action: action,
    ...
    exportExcel(action) {
      var loading = this.$loading.show();
      this.$store
       .dispatch("alat_kontruksi/exportReport", {
          data: this.selectedItems,
        })
        .then((resp) => {
          loading.hide();
    ...
    exportPDF() {
      this.$router.push({ name: "AlatKontruksiExportPDF", query: { data: this.selectedItems } });
    },
    pagination(page) {
      this.page = page;
